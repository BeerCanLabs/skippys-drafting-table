...
Product engineers describe what they are building to their local AI assistant (Cursor, Claude Code, Copilot, Antigravity), which drafts Software Deployment Patterns (`.draft/sdp.yaml`). During composition, the DRAFT composition engine resolves pre-approved, versioned shared services into real Infrastructure-as-Code (IaC) modules (Terraform, OpenTofu, Pulumi, CloudFormation).
...
## 3. Environment & Landing Zone Abstraction
...
### The DevOps-Owned Solution: `LandingZone` / `EnvironmentProfile`
...
| **Infrastructure Landing Zone** | DevOps / Platform Team | `EnvironmentProfile` / `LandingZone` | Cloud provider accounts, subnet mappings, IAM execution roles, key management (KMS), and ingress routing. |

```yaml
# Example DevOps-Owned LandingZone Specification (in drafting-table/configurations/environments/)
schemaVersion: '1.0'
...
When an engineer runs `compose_iac.py --tier prod`, the engine merges the product's SDP intent with the DevOps-owned `EnvironmentProfile` for `prod`. The product engineer writes zero cloud plumbing, while DevOps retains 100% centralized control over infrastructure landing zones.
